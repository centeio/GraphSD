...
def get_local_outlier_factor_scores(df, k=5, contamination=.1):
...
#         # try:
#         areasp = get_local_outlier_factor(position[['x', 'y']], k=k, contamination=contamination)
#         # print(areasp)
#         # return areasp
#
#         tempareas = []
...
def getAreas(points, pids, print_mode=False):
    ...
    mask = MultiPoint([Point(i) for i in
                       [[minx - margin, miny - margin], [minx - margin, maxy + margin], [maxx + margin, miny - margin],
